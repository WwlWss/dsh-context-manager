...
M5C therefore owns a separate process-local request-series coordinator, but steady-state fencing is **contribution-driven**, not event-driven. `dsh-context-manager/change`, `skills/change`, and `system-prompt/change` are invalidation/observation signals; the latter two are unfiltered Host-wide notifications and must not force every Agent into a new request series. They only mark that Agent runtime's final prompt projection dirty. On its next accepted pre-step, the contributor may lazily perform one native diagnostic assembly without a request signal to observe the true post-waterfall/post-`complete` result, but only when the real request assembly's revision, effective Pinned plan, and parent identity are still unchanged. If any of those changed after the real assembly was built, the diagnostic result must not stand in for that request; reconciliation is deferred to the next real assembly. Explicit force revisions are reserved for attach/resume and retirement cleanup; they coalesce with a real fingerprint change into one boundary.
