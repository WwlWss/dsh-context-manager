...
7. After the final awaited catalog/body operation, re-read the effective profile and require the same profile id, exact base-preset identity, and code-unit-sorted Pinned skill-name set that started the assembly. This final no-await coherence fence is necessary because Off and Pinned intentionally share the same native `false / false` invocation policy, so the CM proxy summary alone cannot prove the current mode is still Pinned.
