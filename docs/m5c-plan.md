...
M5C therefore owns a separate process-local request-series coordinator, but steady-state fencing is **contribution-driven**, not event-driven. Payload-free Context Manager / SkillRegistry notifications never force a request series and are not needed to dirty M5C projection state because the next real request assembly already re-reads profile, parent, catalog, and bodies. Only `system-prompt/change` dirties the per-Agent final projection, because native `complete` suppression is restored outside the M5C waterfall and therefore cannot be observed directly by the request assembly listener. On the next accepted pre-step, the contributor may lazily perform one native diagnostic assembly without a request signal to observe that true post-waterfall/post-`complete` result, but only when the real request assembly's revision, effective Pinned plan, and parent identity are still unchanged. If any of those changed after the real assembly was built, the diagnostic result must not stand in for that request; reconciliation is deferred to the next real assembly. Explicit force revisions are reserved for attach/resume and retirement cleanup; they coalesce with a real fingerprint change into one boundary.
...
The first request after attaching/re-attaching M5C is explicitly fenced, so a resumed 0.1.2+ Session does not depend on process-local knowledge of its prior pinned state. Only signatures/booleans are retained between steps; full Skill bodies are never cached. An unchanged final CM contribution does not force a new request series even when an unrelated Host-wide registry notification fires.
...
6. After all body loads finish, take one **final complete Agent-view Skill snapshot** and revalidate every loaded binding against the current M5B Pinned proxy winner. A loaded body whose final winner is missing, competing, or invocable is discarded and reported as `policy-not-effective`; an incomplete final catalog fails the whole bundle closed. This final Agent snapshot is the last async commit point.
7. Keep only the rendered blocks whose final Agent-view policy remains effective; rendering itself uses DSH native `renderSkillContent()` so resource hints, provider identity, escaping, and canonical `<skill_content>` framing remain native-owned.
8. After that final Agent snapshot, re-read the effective profile **and** the Agent's dynamic parent scope. Require the same profile id, exact base-preset identity, code-unit-sorted Pinned skill-name set, and parent identity that started resolution. After downstream prompt waterfall listeners return, perform the same no-await profile/parent check once more before the final assembly is returned; if it no longer matches, clear the CM slot/variable from that assembly. These coherence fences are necessary because Off and Pinned intentionally share the same native `false / false` invocation policy and because a live Agent can be re-parented without recreation; neither the CM proxy summary nor the original parent snapshot proves the final assembly still belongs to the same effective view.
9. Join the surviving rendered blocks deterministically into one Context Manager bundle.
10. Replace the one fixed CM slot with that bundle. An empty bundle removes the slot from the current assembly.
