...
7. After the final awaited catalog/body operation, re-read the effective profile and require the same profile id, exact base-preset identity, and code-unit-sorted Pinned skill-name set that started the assembly. After downstream prompt waterfall listeners return, perform the same no-await check once more before the final assembly is returned; if it no longer matches, clear the CM slot/variable from that assembly. These coherence fences are necessary because Off and Pinned intentionally share the same native `false / false` invocation policy, so the CM proxy summary alone cannot prove the current mode is still Pinned.
