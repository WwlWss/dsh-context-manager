...
M5C therefore owns a separate process-local request-series coordinator, but steady-state fencing is **contribution-driven**, not event-driven. `dsh-context-manager/change`, `skills/change`, and `system-prompt/change` are invalidation/observation signals; the latter two are unfiltered Host-wide notifications and must not force every Agent into a new request series. They only mark that Agent runtime's final prompt projection dirty. On its next accepted pre-step, the contributor lazily performs one native diagnostic assembly without a request signal, observes the true post-waterfall/post-`complete` result, and compares that final CM-owned SHA-256 fingerprint with the durable admitted baseline. Explicit force revisions are reserved for attach/resume and retirement cleanup; they coalesce with a real fingerprint change into one boundary.
