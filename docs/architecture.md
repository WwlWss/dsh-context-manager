...
M5B's shipped policy behavior is verified across live Agent creation/adoption/disposal, cold/resumed sessions where a public seam applies, dynamic preset-scope reparenting, provider invalidation, exact base-preset switching, duplicate-name precedence, HMR/unload restoration, and model/user invocation leakage. Catalog/list paths must remain metadata-only; load full bodies lazily through provider `get()`. Coalesce one Context Manager authority change to at most one registry-wide Skill invalidation rather than invalidating once per Agent. Do not create a `skills/change` feedback loop: Context Manager Settings changes are its authority invalidation source, while native SkillRegistry invalidation already owns catalog refresh.
