...
Runtime Prompt registration is a small fixed Agent-scoped placeholder set, not one DSH registration per PromptResource: four empty system-section placement slots, one empty runtime-context slot, and one scoped assembly waterfall expander. The expander resolves the current effective profile/resources once and replaces placeholders with independent per-binding assembled contributions before delegating to the remaining native waterfall. Independent contributions preserve native interpolation boundaries; a single resolving callback removes the need for an `AssembleContext` memoization cache. No cross-step cache may become a second authority for Settings, Prompt Library, Session identity, or native composition state.
...
Pinned full instructions use native `renderSkillContent()` and one owned replacement/clearable system-prompt bundle; they are never appended as per-step user messages. Legacy prompt interpolation is bypassed safely by substituting the already-rendered bundle through one scoped prompt variable, whose value is not rescanned. After asynchronous Skill discovery/body loading, M5C re-reads both the effective profile and the Agent's dynamic parent scope, and repeats that coherence check after downstream prompt assembly. It fails closed unless the same profile/base-preset/Pinned-name plan and parent identity are still current; this prevents an Off proxy from being mistaken for Pinned merely because both modes deliberately expose `false / false` native invocation, and prevents a re-parented Agent from receiving a body resolved from its old parent. On retained 0.1.2+ lines that expose public `startsRequestSeries`, Context Manager owns only a request-series fence whose steady-state authority is the final per-Agent CM contribution fingerprint. Unfiltered `skills/change` / `system-prompt/change` notifications do not themselves fence Agents. Skill/profile changes are observed by the next real request assembly and do not dirty M5C projection state; only `system-prompt/change` marks the per-Agent final projection dirty because native post-waterfall `complete` suppression otherwise needs one final observation. The next pre-step may lazily re-observe one native signal-free assembly so post-waterfall `complete` restoration is included before comparison, but only if the already-built real request still matches the same revision/effective Pinned plan/parent identity. A later authority or parent change cannot let a diagnostic assembly advance admission past the contribution DSH is actually about to send. Explicit force is reserved for attach/resume and retirement cleanup. The pre-step decision prepares the boundary but does not commit CM state; the matching public durable `request/header` Session event acknowledges admission and advances the baseline after `agent/request` / `prepareCall()` have succeeded. DSH then owns stale system-node reconciliation. Retained 0.1.1 predates that public field and remains correct on its tested legacy prompt path. Context Manager does not write synthetic core Session events or version-shaped Surface replacements for M5C, and total bundle removal therefore makes no cross-resume cleanup guarantee for prompt bytes admitted before unload.
