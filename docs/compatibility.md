...
Real AgentLoop coverage includes the legacy 0.1.1 path, 0.1.5-rc.1 as the first retained `systemPromptUpdate: 'in-history'` path, and the current 0.1.6-alpha.2 path. M5C fingerprints only its final pinned contribution and uses a longer-lived request-series fence to ask native DSH to consolidate stale system nodes when Pinned state/body/visibility changes or the contribution hot-unloads. M5C does not own direct Session Surface replacements.
