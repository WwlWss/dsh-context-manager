...
Development generation is pinned to the oldest retained `0.1.1-rc.2` Typert generator/protocol contract. CI generates one strict Host/Remote artifact and then runs that artifact against the real Typert Registry and API Gateway from `0.1.1-rc.2`, `0.1.2-rc.1`, `0.1.5-rc.1`, `0.1.5-rc.2`, and `0.1.6-alpha.2`. A runtime result obtained only through Gateway SRC fallback is not sufficient.
