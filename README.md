...
6. **Complete** — strict Host Remote API: M6A foundation, M6B profiles/Prompt Resources, and M6C preset/runtime diagnostics with pull change hints are complete.
7. **In progress** — M7A Client artifact/loader/Remote/Slot foundation and M7B0 retained Client presentation-contract cleanup are complete; M7B1 is next and adds the authoritative Client model, followed by M7B2 revision-aware mutations, M7C Profile Drawer, and M7D browser/package compatibility closeout.
8. Focused PromptResource/PromptBinding, Skill policy, native preset, effective-preview, and advanced stored-payload editors.
9. Project and Session bindings, beginning with a public persistence/ownership contract probe rather than assuming global Settings can emulate those scopes.
10. Transform/resource milestones, history-Surface transforms (beginning with a Session/Surface extension contract probe), display transforms, renderer/helper runtime, import/export, and advanced inspection follow in the detailed roadmap.
...
The git-install `prepare` path emits the complete installable artifact required by the declared package exports, including required declarations, generated Typert faces/contributions, and the Web Client artifact. Full source type checking and non-install test output remain development/CI responsibilities.
