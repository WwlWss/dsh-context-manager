...
6. **In progress** — M6A strict Typert Remote foundation, followed by M6B browser-safe Profile/PromptResource CRUD and M6C preset/runtime diagnostics.
...
The git-install `prepare` path is intentionally smaller than the development build: it transpiles runtime JavaScript and generates the declared Typert `./typert` / `./remote` artifacts, but skips the main declaration build and full type checking. Those remain development/CI responsibilities.
