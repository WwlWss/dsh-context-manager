...
export function parsePromptOrder(value: unknown): number {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new TypeError('prompt order must be a finite number')
  }
  return value
}
...
export function parsePromptBinding(raw: unknown): PromptBinding {
  ...
  if (typeof raw.enabled !== 'boolean') throw new TypeError('prompt binding.enabled must be a boolean')
  ...
  try {
    ...
    throw new TypeError('prompt binding.order must be a finite number')
  }
...
export function parseProfile(raw: unknown): ContextProfile {
  ...
  for (const [resourceId, binding] of Object.entries(raw.prompts ?? {})) {
    try {
      prompts[resourceId] = parsePromptBinding(binding)
    } catch (error) {
      throw new TypeError(
        `profile.prompts[${JSON.stringify(resourceId)}] ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  }
