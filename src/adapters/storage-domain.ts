...
export interface PromptStorageDomainSpec<V> {
  ...
  readonly tables: Readonly<Record<string, { readonly valueSchema: { parse(value: unknown): V } }>>
}
...
export async function openPromptStorage<K extends string, V>(ctx: Context, spec: PromptStorageDomainSpec<V>, tableName: string): Promise<PromptStorageHandle<K, V>> {
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError([validationError, cleanupError], 'storageDomain validation and cleanup both failed')
    }
